...
- **Load order matters.** `index.html` lists scripts in dependency order: vendored jQuery → vendored jQuery UI → `namespace` → `persistence` → `widget-registry` → `user-data` → `actions` → `panel` → `workspace` → `menu` → `app` → `widgets/index` → `models` → `main`. Content-widget scripts are **not** listed; they're injected on demand.
...
- `src/core/actions.js` — `Similex.actions`: the single dispatch point for user actions and the recorder that logs them. `dispatch(action, run?)` records an entry `{ seq, ts, type, target, payload, changes }`; if a `run` effect is given, every `userData` change it causes is captured into `changes` (a "transaction"), so the log audits its own data effects and a later undo layer can reverse them. Nested dispatches attribute their changes to the outermost transaction. `subscribe(fn)` observes the stream; `record(on)` toggles recording (effects still run when off — for replay); `log()`/`clear()`/`toJSON()`/`fromJSON(arr)` manage the log. Framework code auto-dispatches what it owns: `menu.js` wraps each selection as `menu.select`; `workspace.js` emits `panel.add`/`panel.close` and, from the panel's classified `onChange`, `panel.move`/`panel.resize`/`panel.minimize`/`panel.maximize`/`panel.ref` — all suppressed during restore (`_suspend`) and `clear()` (`_clearing`). So widget authors get these logged for free.
