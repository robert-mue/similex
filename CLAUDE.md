...
- **Load order matters.** `index.html` lists scripts in dependency order: vendored jQuery → vendored jQuery UI → `namespace` → `persistence` → `widget-registry` → `user-data` → `panel` → `workspace` → `menu` → `app` → `widgets/index` → `main`. Content-widget scripts are **not** listed; they're injected on demand.
